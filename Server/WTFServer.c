...
/*
int compress(char * projectName){
 
  TAR * ptar;
  char * tarFilename = malloc(strlen(projectName) + 4);
  int tar;
  memcpy(tarFilename,projectName, strlen(projectName));
  memcpy(tarFilename[strlen(projectName) + 1], ".tar", 4);
  
  tar = tar_open(&ptar, tarFilename, NULL, O_RDWR, 0777, TAR_GNU);
  if(tar == -1){
    printf("ERROR: %s\n", strerror(errno));
    return -1;
  }
